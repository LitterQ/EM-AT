...
class Attack_None(nn.Module):
    ...
    def forward(self, inputs, targets, attack=None, batch_idx=-1):
        ...
        for j in range(1):
            ...
            x = x - 2/255 * 2 * torch.sign(inp_grad) * 6

        outputs = self.basic_net(x.detach())[0]
    ...
    def forward(self,
        ...
        for i in range(self.num_steps):
            ...
            if x.grad is not None:
                x.grad.data.fill_(0)
            aux_net.eval()
            logits, fea_energy = aux_net(x)
            loss = self.loss_func(logits, y_tensor_adv)
            loss_adapt = fea_energy.mean()
            loss = 1 * loss.mean() - 100 * loss_adapt
            aux_net.zero_grad()
            loss.backward()
...
class Attack_FeaScatter(nn.Module):
    ...
    def forward(self,
        ...
        total_loss = mse + adv_loss + nat_loss
